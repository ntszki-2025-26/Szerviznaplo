...
<div class="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <div class="mb-6 sm:mb-8">
        ...
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Felhasználók kezelése</h1>
    </div>

    @if(session('success'))
        <div class="flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 mb-6 text-sm">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" class="shrink-0"><polyline points="20 6 9 17 4 12"/></svg>
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
                            ...
                            <td class="px-6 py-4">
                                <form action="{{ route('admin.users.role', $user) }}" method="POST">
                                    ...
                                    <select name="role" onchange="this.form.submit()"
                                        ...
                                        <option value="{{ App\Models\User::ROLE_MECHANIC }}" @selected($user->role === App\Models\User::ROLE_MECHANIC)>🔧 Mechanic</option>
                                        <option value="{{ App\Models\User::ROLE_ADMIN }}" @selected($user->role === App\Models\User::ROLE_ADMIN)>🛡️ Admin</option>
                                    </select>
                                </form>
                            </td>
                            <td class="px-6 py-4">
        ...
        <div class="sm:hidden divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <div class="px-4 py-4 space-y-3">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="text-xs text-red-400 border border-red-400/30 px-3 py-1.5 rounded-lg hover:bg-red-400/10 transition-colors shrink-0">
                                Törlés
                            </button>
                        </form>
                    </div>
                    ...
                    <form action="{{ route('admin.users.role', $user) }}" method="POST">
                        @csrf
                        <select name="role" onchange="this.form.submit()"
                            class="w-full text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400 cursor-pointer">
                            <option value="{{ App\Models\User::ROLE_USER }}" @selected($user->role === App\Models\User::ROLE_USER)>👤 User</option>
                            <option value="{{ App\Models\User::ROLE_MECHANIC }}" @selected($user->role === App\Models\User::ROLE_MECHANIC)>🔧 Mechanic</option>
                            <option value="{{ App\Models\User::ROLE_ADMIN }}" @selected($user->role === App\Models\User::ROLE_ADMIN)>🛡️ Admin</option>
                        </select>
            ...
            @empty
                <div class="px-4 py-12 text-center text-gray-400 text-sm">Nincs felhasználó</div>
            @endforelse
        </div>
    </div>
</div>
